...
class AgentRuntime:
    ...
    async def list_skills(self) -> list[SkillInfo]:
        await self._ensure_started()
        assert self.paths
        return [SkillInfo(**entry) for entry in build_skill_catalog(self.paths.home_dir)]
    ...
    async def load_skill(
        ...
        await self._ensure_started()
        assert self.paths and self.store
        name = name.strip()
        if not name:
        ...
        if session_id in self._session_active or self._session_queues.get(session_id) or await self.store.has_active_runs(session_id):
            return LoadSkillResult(
                session_id=session_id,
                name=name,
                loaded=False,
        ...
        entry = find_skill_by_name(self.paths.home_dir, name)
        if entry is None:
            return LoadSkillResult(
                session_id=session_id,
                name=name,
        ...
        run_mode = RunMode(mode)
        session = await self.store.get_session(session_id)
        agent_id = str(session["root_agent_id"]) if session is not None else self._root_agent_id(session_id=session_id, mode=run_mode)
        session_created = await self.store.ensure_session(session_id=session_id, cwd=str(self.paths.project_dir), root_agent_id=agent_id)
        await self.store.ensure_agent(
            agent_id=agent_id,
            session_id=session_id,
        ...
        already_loaded = self._context_state_for_session(session_id).mark_skill(name, path, body)
        ...
        parent_id = await self.store.active_node_id(session_id)
        node = await self.store.add_node(
            session_id=session_id,
            parent_id=parent_id,
            agent_id=agent_id,
            run_id=None,
